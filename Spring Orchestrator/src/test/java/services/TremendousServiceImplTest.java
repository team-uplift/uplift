...
class TremendousServiceImplTest {
    ...
    void testSubmitDonationOrder_Success() {
        ...
        String result = tremendousService.submitDonationOrder(recipient, donor, donationAmount);

        assertNull(result); // Method returns null, ensuring no exceptions were thrown
    }
    ...
    void testSubmitDonationOrder_BadRequest() {
        ...
        when(responseSpec.bodyToMono(TremendousOrderResponse.class)).thenReturn(Mono.error(new RuntimeException("Bad request")));

        Exception exception = assertThrows(RuntimeException.class, () -> tremendousService.submitDonationOrder(recipient, donor, donationAmount));
