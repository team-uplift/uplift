...
public class BedrockServiceImpl implements BedrockService {
	...
	public Map<String, Double> getTagsFromPrompt(String prompt) {
		String finalPrompt = "Only generate a comma separated list of tags/descriptors and associated "
				...
				+ prompt;
		String response = ChatClient.create(chatModel).prompt().user(u -> u.text(finalPrompt)).call().content();
	...
	/**
	 * This method will implement the LLM recipient matching based on the tag generation
	 * with a simple rag based prompting.
	 * @param prompt
	 * @return
	 */
	@Override
	public List<String> matchTagsFromPrompt(String prompt) {
		...
		// Implement the injected rag content to filter the output and isolate responses
		// to specifically known tags.
