...
public interface RecipientRepository extends JpaRepository<Recipient, Integer> {
	 ...
	 * @return List of recipients who have selected the specified tag or an empty
	 *         list if
	...
	/**
	 * Loads all recipients that are linked to the specified tag, whether they
	 * selected it
	 * or not.
	 * 
	 * @param tag name of the tag to which the recipient must be linked (selected or
	 *            not)
	 * @return List of recipients who are linked to the specified tag or an empty
	 *         list if
	 *         there are no such recipients
	 */
	@Query("""
			    ...
			    WHERE RecipientTag.tagName IN :tags
			    GROUP BY Recipient.id
			    ORDER BY COUNT(RecipientTag.tagName) DESC, Recipient.createdAt ASC
			""")
